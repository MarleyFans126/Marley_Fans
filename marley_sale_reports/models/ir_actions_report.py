...
class IrActionsReport(models.Model):
    ...
    def _marley_stamp_print(self, report_ref, res_ids):
        ...
        _logger.info("Marley stamp: called report_ref=%s res_ids=%s", report_ref, res_ids)
        if not res_ids:
            return
        try:
            ...
            return
        if not report:
            _logger.info("Marley stamp: report is None")
            return
        _logger.info(
        ...
        try:
            ...
            if orders:
                ...
                _logger.info(
                    "Marley stamp: STAMPED last_print_date=%s on sale.order ids=%s",
                    now, orders.ids,
                )
        except Exception as e:
