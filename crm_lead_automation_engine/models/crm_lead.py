...
class CrmLead(models.Model):
    ...
    # When a customer emails a lead — a reply to one of our mails OR a fresh
    # email routed to info@ that lands on a lead — forward a copy to the
    # operations mailbox AND to the assigned salesperson, so it reaches their
    # own inbox on top of appearing in the lead chatter.
    #
    # Fires ONLY for genuine inbound gateway mail (message_type == 'email').
    # Outbound composer messages and this module's auto-mails are posted as
    # 'comment'/'notification', so they never trigger a forward — no loops.
    # The forward goes to operations@ / salesperson (never back to info@),
    # so it cannot re-enter the mail gateway.
    # -------------------------------------------------------------------------
    def _ops_forward_enabled(self):
        """Master toggle (config: crm_automation.forward_incoming_email)."""
        val = self.env['ir.config_parameter'].sudo().get_param(
            'crm_automation.forward_incoming_email', 'True')
        return str(val).strip().lower() not in ('false', '0', '', 'none')

    def _ops_forward_address(self):
        """Fixed operations mailbox (config: crm_automation.forward_ops_email)."""
        return (self.env['ir.config_parameter'].sudo().get_param(
            'crm_automation.forward_ops_email', 'operations@marleyfans.in') or '').strip()

    def message_post(self, **kwargs):
        message = super().message_post(**kwargs)
        if (kwargs.get('message_type') == 'email'
                and not self.env.context.get('mail_ops_forwarding')
                and self._ops_forward_enabled()):
            try:
                self.with_context(mail_ops_forwarding=True).sudo() \
                    ._forward_incoming_email_to_ops(message)
            except Exception as e:
                _logger.warning(
                    "Ops-forward failed for lead %s (msg %s): %s",
                    self.ids, getattr(message, 'id', None), e)
        return message

    def _forward_incoming_email_to_ops(self, message):
        """Forward one inbound customer email to operations + salesperson(s)."""
        self.ensure_one()
        if not message:
            return

        # Defensive: inbound gateway mail is authored by the customer. If the
        # author is one of our own internal (non-share) users, it is not an
        # incoming customer email — skip.
        author = message.author_id
        if author and author.user_ids and all(not u.share for u in author.user_ids):
            return

        root_user = self.env.ref('base.user_root', raise_if_not_found=False)

        recipients = []
        ops = self._ops_forward_address()
        if ops:
            recipients.append(ops)
        # Assigned salesperson + second salesperson (skip portal/share users,
        # OdooBot, and anyone without an email).
        for sp in (self.user_id | self.second_salesperson_id):
        ...
        sender = message.email_from or (author.email_formatted if author else '')
        header = Markup(
            ...
            '<strong>From:</strong> %(sender)s'
            '</div>'
        ) % {'lead': self.name or '', 'sender': sender or _('(unknown)')}
        body = header + Markup(message.body or '')

        subject = message.subject or (_("Customer email — %s") % (self.name or ''))
        if not subject.strip().lower().startswith('fwd'):
            subject = 'Fwd: %s' % subject

        # Copy attachments so the forward is self-contained; originals on the
        # lead chatter are left untouched even though the forward auto-deletes.
        attachment_cmds = [
        ...
        mail = self.env['mail.mail'].sudo().create({
            'subject': subject,
            'body_html': body,
            'email_from': self.env.company.email or ops,
            'email_to': ', '.join(email_to_list),
            'auto_delete': True,
            'attachment_ids': attachment_cmds or False,
        })
        mail.send()
        _logger.info(
            "Ops-forward: lead %s inbound email forwarded to %s",
            self.id, ', '.join(email_to_list))
